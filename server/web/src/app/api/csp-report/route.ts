...
const BACKEND_URL =
  ...
  process.env.NEXT_PUBLIC_MANAGEMENT_SERVER_URL ||
  'http://localhost:8766';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const entries = violations.map((v) => ({
      ...
      file: v.sourceFile,
      line: v.lineNumber,
      metadata: { component: 'csp-report', app: 'operations-console', ...v },
    }));
    ...
    await fetch(`${BACKEND_URL}/api/logs`, {
      ...
      headers: {
        ...
        'X-Client-ID': 'web-csp-report',
        'X-Client-Name': 'Operations Console CSP Reporter',
      },
