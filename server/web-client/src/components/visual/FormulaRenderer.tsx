...
// ===== KaTeX trust policy =====
//
// Curriculum LaTeX is untrusted: it can be imported from external sources or
// generated by an LLM. KaTeX trust:true would enable \href/\url with arbitrary
// protocols (javascript:, data:) and the raw-HTML \html* family, which is a
// stored/DOM XSS sink in a learner's session. This callback permits only
// \href/\url to safe protocols and rejects every other trust-gated command.
const SAFE_KATEX_PROTOCOLS = ['http', 'https', 'mailto', '_relative'];

function katexTrust(context: { command: string; protocol?: string }): boolean {
  if (context.command === '\\href' || context.command === '\\url') {
    return !!context.protocol && SAFE_KATEX_PROTOCOLS.includes(context.protocol);
  }
  return false;
}
