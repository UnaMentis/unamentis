...
class TestInMemoryRateLimiter:
    ...
    @pytest.mark.asyncio
    async def test_first_request_from_new_client_is_allowed(self, strict_limiter):
        """Regression: a brand-new client's very first request must be allowed.

        New buckets are seeded full (tokens == burst). Previously they started
        empty (tokens == 0), which 429'd every new client on their first
        login/registration.
        """
        allowed, headers = await strict_limiter.check_rate_limit(
            "brand-new-key", "test"
        )
        assert allowed is True

    @pytest.mark.asyncio
    async def test_first_request_allowed_for_strict_auth_categories(self):
        """The strict auth/login and auth/register limits must allow the first hit."""
        limiter = InMemoryRateLimiter()
        for category in ("auth/login", "auth/register"):
            allowed, _ = await limiter.check_rate_limit(
                f"ip:1.2.3.4:{category}", category
            )
            assert allowed is True, f"first {category} request was denied"
        ...
        limiter = InMemoryRateLimiter(
            {
                ...
                "unauthenticated": RateLimitConfig(
                    requests=1, window_seconds=60, burst=1
                ),
            }
        )
